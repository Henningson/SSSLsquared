...
def main():
    parser = Args.GlobalArgumentParser(
        ...
        description="Train a Segmentation Network that simultaneously outputs keypoints",
    ...
    TRAIN_TRANSFORM_PATH = (
        ...
        if LOAD_FROM_CHECKPOINT
        else "configs/train_transform.yaml"
    )
    VAL_TRANSFORM_PATH = (
        os.path.join(CHECKPOINT_PATH, "val_transform.yaml")
        if LOAD_FROM_CHECKPOINT
        else "configs/val_transform.yaml"
    )
    ...
    # Save config stuff
    A.save(
        train_transform, CHECKPOINT_PATH + "/train_transform.yaml", data_format="yaml"
    )
            ...
            wandb.log(
                {
                    "{0} {1}".format(title, i): wandb.Image(
                        images[i].detach().cpu().moveaxis(0, -1).numpy(),
                        masks={
                            "predictions": {
...
def generate_video(model, data_loader, path):
    ...
    for images, gt_seg, _ in data_loader:
        ...
        visualizer = Visualizer.Visualize2D(
            x=1, y=1, remove_border=True, do_not_open=True
        )
    ...
    out = cv2.VideoWriter(
        ...
        10,
        (video_list[0].shape[1], video_list[0].shape[2]),
    )
