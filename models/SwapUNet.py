...
class ConcatBatches(nn.Module):
    def __init__(self):
        super(ConcatBatches, self).__init__()

    def forward(self, x):
        nums = list(range(0, x.shape[0]))
        nums = nums[-1:] + nums[:-1]
        return torch.concat([x, x[nums, :]], dim=1)
...
class Model(nn.Module):
    def __init__(self, config={'in_channels': 3, 'out_channels': 4, 'features': [64, 128, 256, 512], 'batch_size': 4}, pretrain=False, state_dict=None, device="cuda"):
        ...
        self.concatenator = ConcatBatches()
        self.bottleneck_conv = nn.Conv2d(self.bottleneck_size*2, self.bottleneck_size, kernel_size=1)
    ...
    def load_from_dict(self, dict):
        ...
        self.bottleneck_conv.load_state_dict(dict["BottleneckConv"])
        try:
...
def test():
    ...
    batch_size = 4
    sequence_length = 6
    config={'batch_size': batch_size, 'in_channels': 3, 'out_channels': 4, 'features': [64, 128, 256, 512], 'sequence_length': sequence_length}
