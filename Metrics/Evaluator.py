...
class F1ScoreMetric(BaseMetric):
    ...
    def compute(self, prediction, target) -> float:
        TP, FP, FN, _ = KeypointMetrics.keypoint_statistics(prediction, target, 
...
class AveragePrecisionMetric(BaseMetric):
    ...
    def get_final_score(self):
        return KeypointMetrics.average_precision(self.TOTAL_TP, self.TOTAL_FP, self.TOTAL_FN)
...
class NMEMetric(BaseMetric):
    ...
    def compute(self, prediction, target) -> float:
        _, _, _, inlier_distances = KeypointMetrics.keypoint_statistics(prediction, target, 
                                                        self.outlier_threshold, 
                                                        prediction_format=self.prediction_format, 
                                                        target_format=self.target_format)
